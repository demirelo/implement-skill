...
Simplest path: set **one** `OPENROUTER_API_KEY` env var — it fronts the OpenRouter models in the
pool, no 1Password needed. Grok is available through `~x-ai/grok-latest`; DeepSeek, MiniMax, and
Kimi remain available as Builder choices.
...
Models backed by the running host, such as orchestrator-only Codex routes, use the host callback
seam; script-dispatchable models use the configured pool and credential sources.
