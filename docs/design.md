...
- `/solve` — research (a sibling skill; not shipped in this repo)
...
| Team | Models | Owns |
|---|---|---|
| **Architects** | Claude (Opus 4.8) · GPT xhigh (GPT-5.6 Sol / Codex via MCP) · GLM (5.2) | **Phase 0 intent** + **planning** (multi-round consensus) + **writing the acceptance tests** + **adversarial review** |
...
Reuse aggressively — `team_dispatch.py`, `providers.json`, and the ledger discipline are adapted from `/solve`, a sibling research skill not shipped here.
