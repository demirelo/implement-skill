...
**human-in-the-loop, two-team** model org and a **GitHub PR** as the deliverable.
...
principles that distinguish this approach: an objective oracle, a human merge gate, and local
sovereignty.
...
GLM is promoted from `/solve`'s cost tier to Architect-tier. Builder models are reached via the existing
`~/.claude/skills/solve/scripts/team-dispatch.py` (1Password keys per-call, OpenAI-compatible,
...
PHASE 5 — HAND OFF
  Flip to ready-for-review; tier 🟢/🟡/🔴; structured PR body (goal, plan + consensus notes,
  k/N acceptance green, test evidence, curated review, "decisions needed / blocked / risks").
  → HUMAN MERGES. The loop NEVER self-merges.
```
...
- **Human touchpoints are exactly two** (plus one optional): Phase 0 intent-confirm (required) and the final merge (required); Phase 1 plan-approval is optional/off by default. Everything else — local gates, draft-PR creation, review, fix, handoff — is **fully automated, no human attention**.
- **Phase 0 intent gate** — no spend until intent is confirmed.
- **No auto-merge** — PRs always left for human approval.
...
- **Senior-human-in-the-loop ceiling** — best on greenfield, well-specified work; human owns the last ~10%.
...
| **Human role** | a **collaborator** that leaves a reviewable PR; exactly 2 touchpoints; never self-merges. |
...
you own, with the human at the merge gate. The oracle under the hood and the human at the door are the
whole point.
