...
def overlay_profile_credentials(cfg, home=None):
    ...
    in ~/.config/implement/config.json (written by setup.py). Overlay op-source credential specs so
    dispatch resolves the same credentials readiness validated. env-source creds need no overlay
    (env_read sees them); dotenv/keychain API-key sources are not implemented here (v1)."""
    ...
    for provider, spec in creds.items():
        if spec.get("source") != "op" or provider not in cfg or not spec.get("ref"):
            continue
        entry = cfg[provider]
        entry["key_ref"] = spec["ref"]
        if spec.get("account"):                      # never clobber with an empty account
        ...
        if spec.get("service_account_keychain_service"):
            entry["service_account_keychain_service"] = spec["service_account_keychain_service"]
    return cfg
