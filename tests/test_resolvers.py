...
def test_resolve_op_keeps_account_without_service_token(monkeypatch):
    # isolate from the dev machine: an ambient OP_SERVICE_ACCOUNT_TOKEN legitimately drops
    # --account (see test_resolve_op_drops_account_when_token_only_in_os_environ), so the
    # no-token case must guarantee there IS no token anywhere.
...
def test_resolve_op_with_none_runner_falls_back_to_subprocess(monkeypatch):
    # smoke.py --live returns runner=None into readiness -> resolve(runner=None). An op/keychain
    # source must NOT try to call None (TypeError: 'NoneType' object is not callable) — it falls
    ...
    monkeypatch.setattr(resolvers.subprocess, "run", lambda *a, **k: (calls.append(a), P())[1])
    cred = resolve({"source": "op", "ref": "op://v/i/credential"}, env={}, runner=None)
    assert cred == Cred(key="sk-live", source="op") and calls   # used subprocess.run, never called None


def test_resolve_keychain_with_none_runner_falls_back_to_subprocess(monkeypatch):
    import resolvers
    ...
    cred = resolve({"source": "keychain", "service": "x"}, env={}, runner=None)
    assert cred == Cred(key="sk-kc", source="keychain")


def test_validate_with_none_runner_falls_back_to_subprocess(monkeypatch):
    import resolvers
    ...
    monkeypatch.setattr(resolvers.subprocess, "run", lambda *a, **k: P())
    assert validate(["true"], runner=None) is True
