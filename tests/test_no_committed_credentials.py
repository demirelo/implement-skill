...
fails the build if a real 1Password id, a key value, or a non-placeholder `op://` vault ever lands in
tracked config. Real credentials belong in ~/.config/implement via setup.py
...
def test_tracked_json_config_is_template_only():
    # scan the actual STRING VALUES (incl. _comment — a real ref can hide in a comment),
...
def test_no_real_1password_ids_in_any_tracked_text():
    ...
    # appear in ANY tracked file — config, docs, or knowledge-base. This catches such
    # coordinates reappearing anywhere by SHAPE, so the guard never has to memorize (and thus
    # re-embed) any specific string.
