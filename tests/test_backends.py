...
def test_make_dispatcher_with_none_runner_falls_back_to_subprocess(monkeypatch):
    # smoke.py --live threads runner=None through run_implement into the dispatchers; the
    # dispatch fn must fall back to subprocess.run, never call None (readiness had the same
    # bug — fixed in resolvers; this is the dispatch-side twin).
    import backends
    ...
    monkeypatch.setattr(backends.subprocess, "run", lambda *a, **k: P())
    entry = {"backend": "team_dispatch", "provider": "deepseek", "route": "openrouter", "data": "standard"}
    fn = backends.make_dispatcher(entry, runner=None)
    assert "+b" in fn("prompt")
