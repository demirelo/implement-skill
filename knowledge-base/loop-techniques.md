...
adversarial loop for a given user intent**. It generalizes `/solve`, a sibling research loop not shipped in this repo: the engine is
...
failure such a research loop is built to catch. **Rule: any `source_authority` star count must be re-fetched
...
|---|---|---|---|
| Conductor over parallel sub-teams (directions vs angles) | sibling research loop | research, general | One conductor decides over 2–4 *genuinely different* directions; workers only propose. |
...
| technique | source | domains | insight |
|---|---|---|---|
| Honest 3-mode metric + cost-tiered routing | sibling research loop | research, data-analysis | Pick the measure by task shape (coverage% / distance-coordinate / calibrated odds); count only gate-verified progress; let it drop honestly; cap combined odds at the shared bottleneck; buy breadth cheap behind a frontier verification net. |
...
| technique | source | domains | insight |
|---|---|---|---|
| Cross-family refutation-survival | sibling research loop | research, code-review | Consensus = survives cross-family refutation + passes a ground-truth check + every citation resolves. Route disagreement to the crux. |
...
| Glossary / CONTEXT.md shorthand | Matt Pocock-style (via a sibling research loop) | general | Shared domain vocabulary replaces paragraphs; cheaper *and* more consistent workers. |
...
- **autonomy_loop / guardrails:** worktree isolation; destructive-command gating hook; Plan/Act + per-category gates; stop-hook re-feed; suitability filter; denial caps + reasoning-blind classifier; **draft PR → review comments on PR → tiered; green-gated auto-merge (🟢 self-merges, 🟡/🔴 fall back to a human handoff; branch protection never bypassed).**
...
## Recipe: `research` (the `/solve` door — a sibling skill, not shipped here)
