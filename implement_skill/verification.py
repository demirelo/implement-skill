...
class VerificationContext:
    ...
    def _sanitize_env(self, source: dict) -> dict[str, str]:
        ...
        # Candidate copies intentionally exclude pre-existing ``__pycache__`` directories.  Keep
        # verification from recreating timestamp-based caches so a same-size source edit in the
        # same mtime tick can never execute stale bytecode.
    ...
    def run_argv(self, argv, *, cwd=None, timeout=None, check=False, **kwargs):
        ...
        if scheduler is None:
            ...
            # Gate/oracle commands all pass through this single boundary, including authored RED
            # checks.  Acquiring here (rather than around run_full_gate) avoids nested semaphore
            # acquisition when a gate has multiple phases and keeps candidate threads bounded.
            with scheduler.verification_slot():
                proc = self.runner(self.wrap(argv, workdir=workdir), **options)
    ...
    def run_gate(self, adapter=None, *, only=None, repo_root=None):
        ...
        def metered_runner(argv, *args, **kwargs):
            # ``gate.run_gate`` executes each phase through its injected runner rather than
            # ``run_argv``. Keep this process boundary on the same global verification budget as
            # full/scoped/oracle commands. The wrapper is per phase, so one long gate does not
            # monopolize the CPU slot while another candidate is idle between phases.
            if scheduler is None:
                return self.runner(argv, *args, **kwargs)
            with scheduler.verification_slot():
                return self.runner(argv, *args, **kwargs)

        return gate.run_gate(str(workdir), adapter or self.adapter,
                             wrap=self._gate_wrap, only=only, env=dict(self.env),
                             runner=metered_runner)
